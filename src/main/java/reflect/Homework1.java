...
public class Homework1 {
    ...
    public void streamPipeline3() {
        Arrays.stream(java.lang.String.class.getDeclaredMethods())
                .filter(S-> S.getParameterCount() >= 2 && Arrays.stream(S.getParameterTypes())
                        .allMatch(par-> par == Arrays.stream(S.getParameterTypes()).findFirst().get()))
                .sorted(Comparator.comparing(S-> S.getName()))
                .forEach(System.out::println);
    }
    ...
    public String streamPipeline8() {
        return Arrays.stream(java.lang.String.class.getDeclaredMethods())
                .filter(S-> S.getModifiers() == Modifier.PUBLIC)
                .map(S-> S.getName())
                .max(Comparator.comparingInt(S-> S.length()))
                .get();
    }
